...
def get_args():
    parser = argparse.ArgumentParser(description='T5 fine-tuning loop')

    parser.add_argument('--finetune', action='store_true',
                        help='Whether to fine-tune T5 (use pretrained weights)')
    ...
    parser.add_argument('--experiment_name', type=str, default='t5_ft_experiment')
...
# ---------------------------------------------------------------------------
# FIX: train_epoch — use model's built-in loss via the `labels` argument
# ---------------------------------------------------------------------------
# The original code used nn.CrossEntropyLoss on manually-shifted decoder
# targets.  That is both error-prone and inconsistent with how T5 internally
# handles the decoder start token and EOS.  Passing `labels` to the model
# forward lets HuggingFace do the right shift internally and ignore -100
# padding positions automatically.
# ---------------------------------------------------------------------------
def train_epoch(args, model, train_loader, optimizer, scheduler):
    ...
    for encoder_input, encoder_mask, labels, _, __ in tqdm(train_loader):
        ...
        # Pass labels directly — T5 computes the cross-entropy loss itself
        outputs = model(
            input_ids      = encoder_input,
            attention_mask = encoder_mask,
            labels         = labels,
        )
        loss = outputs.loss   # scalar, already averaged over non-masked tokens

        loss.backward()
        # Gradient clipping prevents exploding gradients (important for T5)
...
def eval_epoch(args, model, dev_loader,
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, labels, _, __ in tqdm(dev_loader):
            ...
            # ---------------------------------------------------------------------------
            # FIX: Generation — do NOT pass decoder_input_ids manually.
            # T5ForConditionalGeneration.generate() sets the decoder start token to
            # config.decoder_start_token_id (= pad_token_id = 0) automatically.
            # Forcing 32099 as the first token breaks greedy decoding.
            #
            # Also increased max_new_tokens to 512 to cover long SQL queries
            # (data stats show max SQL token length = 510).
            # ---------------------------------------------------------------------------
...
def main():
    ...
    # Load best checkpoint for final eval + test inference
    ...
    # -----------------------------------------------------------------------
    # IMPORTANT: the autograder expects exactly these filenames:
    #   t5_ft_experiment_test.sql
    #   t5_ft_experiment_test.pkl
    # -----------------------------------------------------------------------
    test_sql_path    = f'results/t5_ft_experiment_test.sql'
    test_record_path = f'records/t5_ft_experiment_test.pkl'
